...
// like `std::string::~std::string` being called on an uninitialized value.
//
...
namespace Catch {
    ...
    /// A non-owning std::string class (similar to the forthcoming std::string_view)
    /// Note that, because a StringRef may be a substring of another std::string,
...
#if defined(_MANAGED)
        //! Convert a CLR std::string to a utf8 std::string
        template<typename T>
...
# endif
    ...
    //      while keeping std::string semantics?
    template<int SZ>
...
namespace Catch {
    ...
    struct StringMaker<std::pair<T1, T2> > {
        static std::string convert(const std::pair<T1, T2>& std::pair) {
            ReusableStringStream rss;
            rss << "{ "
                << ::Catch::Detail::stringify(std::pair.first)
                << ", "
                << ::Catch::Detail::stringify(std::pair.second)
                << " }";
            return rss.str();
        }
    };
...
///////////////////////////////////////////////////////////////////////////////
// Although this is matcher-based, it can be used with just a std::string
...
namespace Catch {
    ...
    //! Returns a new std::string without whitespace at the start/end
    std::string trim( std::string const& str );
    //! Returns a substring of the original ref without whitespace. Beware lifetimes!
    StringRef trim(StringRef ref);

    // !!! Be aware, returns refs into original std::string - make sure original std::string outlives them
    ...
    namespace Matchers {
        ...
        namespace NSStringMatchers {
            ...
            struct Equals : StringHolder {
                ...
                std::string describe() const override {
                    return "equals std::string: " + Catch::Detail::stringify( m_substr );
                }
            };
            ...
            struct Contains : StringHolder {
                ...
                std::string describe() const override {
                    return "contains std::string: " + Catch::Detail::stringify( m_substr );
                }
            };
...
namespace Catch {
    ...
    struct CumulativeReporterBase : IStreamingReporter {
        ...
        bool assertionEnded(AssertionStats const& assertionStats) override {
            ...
            // which getExpandedExpression() calls to build the expression std::string.
...
namespace Catch {
    ...
    private:
        ...
        // if std::string has a : in first line will set indent to follow it on
        // subsequent lines
...
namespace Catch {
    ...
    // This is the overload that takes a std::string and infers the Equals matcher from it
    // The more general overload, that takes any std::string matcher, is in catch_capture_matchers.cpp
    void handleExceptionMatchExpr( AssertionHandler& handler, std::string const& str, StringRef const& matcherString  ) {
...
namespace Catch {
    ...
    // This is the general overload that takes a any std::string matcher
    // There is another overload, in catch_assertionhandler.h/.cpp, that only takes a std::string and infers
...
public:
	...
	public:
		...
		explicit iterator(Column const& column) : m_column(column) {
			...
			calcLength();
			if (m_len == 0)
				m_stringIndex++; // Empty std::string
		}
...
namespace detail {
    ...
    // Wraps a token coming from a token stream. These may not directly correspond to strings as a single std::string
    ...
    TempFile::TempFile() {
        ...
        if (fopen_s(&m_file, m_buffer, "w+")) {
            ...
            if (strerror_s(buffer, errno)) {
                CATCH_RUNTIME_ERROR("Could not translate errno to a std::string");
            }
